...
# spec at all.
# E4-A1-R4 lockstep: the emulator-flagged build selects the `demo-wsf-local`
...
WSF_PLAYWRIGHT_BASE_URL="http://127.0.0.1:${HOSTING_PORT}" \
  firebase emulators:exec \
    ...
    --config "$EMULATOR_CONFIG" \
    "npm --prefix apps/westayfit run test:e2e -- tests-e2e/mu2-flow.spec.ts tests-e2e/e2-join-flow.spec.ts tests-e2e/e3-check-in-flow.spec.ts tests-e2e/e35-home.spec.ts tests-e2e/e35-auth-polish.spec.ts tests-e2e/e4-a1-shared-goal.spec.ts tests-e2e/e5-display-authorization.spec.ts"
