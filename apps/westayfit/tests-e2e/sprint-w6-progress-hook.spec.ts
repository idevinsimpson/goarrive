...
/**
 * PROGRESS-PARITY-1 — PHASE B: THE REAL ROUTE HOOK (on W9's PERF-MOBILE-1 `889e9775`).
 *
...
test.describe('PROGRESS-PARITY-1 · Phase B · route evidence', () => {
  test('full-viewport route frames beside the frozen reference', async ({ browser }) => {
    ...
    fs.writeFileSync(path.join(dir, 'manifest.json'), `${JSON.stringify({ lovableRef: '09b8a73cc4e661115e52cb1ec4aebcb625c5fc9a', source: 'app/(tabs)/activity.tsx on PERF 889e9775 + ProgressParityView', frames: manifest }, null, 2)}\n`);
  });
});
