...
/**
 * Captures Atlas Batch D — the challenge, and the door — from
 * the gated preview route. TARGETS, not screenshots of a member surface.
 */
const OUT = path.resolve(__dirname, '../../../docs/design-target/review/batch-d-event-and-line');

/*
  OPT-IN, for the reason every capture spec in this suite is. This spec asserts
  nothing about the product — its whole product is the Batch D frames in docs/.
...
test('Atlas Batch D renders at three phone classes, plus its contact sheet', async ({
  browser,
}: {
  browser: Browser;
}) => {
  ...
  try {
    ...
    await page.goto('/design-target/event-queue');
    await page
      .getByTestId('wsf-target-event-queue-batch')
      .waitFor({ state: 'visible', timeout: 30_000 });
    // The wordmark is an image; let it decode before the shutter.
    await page.waitForTimeout(2500);

    await page
      .getByTestId('wsf-contact-batch-d')
      .screenshot({ path: path.join(OUT, 'CONTACT-SHEET-batch-d.png') });

    for (const c of CLASSES) {
      for (const id of SCREENS) {
        await assertStrip(page, `wsf-frame-d-${id}-${c}`, `wsf-frame-banner-d-${id}-${c}`);
        const frame = page.getByTestId(`wsf-frame-d-${id}-${c}`);
