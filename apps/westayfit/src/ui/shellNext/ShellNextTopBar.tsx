...
import { SHELL_TOP_BAR_BODY } from './shellNextMetrics';

/**
 * THE PERSISTENT MEMBER TOP BAR. PROTOTYPE ONLY — this file is not imported by
...
export function ShellNextTopBar({
  onHome,
  menu,
  menuOpen,
  onMenuToggle,
    ...
    >
      ...
      >
        ...
        <Pressable
          ...
          testID="wsf-shell-next-wordmark-home"
        >
          <WsfWordmark variant="navy" height={22} testID="wsf-shell-next-wordmark" />
        </Pressable>
        ...
        <Pressable
          ...
          testID="wsf-shell-next-menu-button"
        >
      ...
      {menuOpen ? (
        <View style={styles.sheet} testID="wsf-shell-next-menu">
          {menu.map((item) => {
            if (item.kind === 'slot') {
              return (
                <View key={item.key} style={styles.slotRow} testID={`wsf-shell-next-menu-slot-${item.key}`}>
            ...
            return (
              <Pressable
                ...
                testID={`wsf-shell-next-menu-${item.key}`}
              >
