 ...
 * truthfully say OPEN; a closed one says only CLOSED.
 *
 ...
 * by its CSS). A closed goal whose total is unknown says only CLOSED: whether
 * it was reached is not known, so it is claimed neither way.
 */
export function statusOf(goal: GoalTruth): LifecycleStatus {
  ...
  if (goal.shared.kind === 'unknown') return { label: 'CLOSED', tone: 'muted' };
  return reached
