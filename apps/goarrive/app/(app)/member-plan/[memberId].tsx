/**
 * Member Plan — Coach full-screen view of a member's plan and questionnaire
 *
 * Accessible at /(app)/member-plan/[memberId]
 * Features:
 *   - Tab 1: Member's intake questionnaire (read-only summary)
 *   - Tab 2: Member's fitness plan (view + edit)
 *   - Edit Mode / Preview Mode toggle for coaches
 *   - Comprehensive inline editing for all plan sections
 *   - GoArrive logo in top-left of plan pages
        ...
        {Array.from({ length: max }).map((_, i) => (
          <Pressable
            ...
            style={{
              ...
              borderWidth: 1,
              borderColor: i < value ? color + '60' : '#1E2535',
            }}
          />
        ))}
      </View>
        ...
        return (
          <Pressable
            ...
            style={{
              ...
              borderRadius: 16,
              borderWidth: 1,
              backgroundColor: isSelected ? color + '20' : '#161B25',
              borderColor: isSelected ? color + '50' : '#2A3347',
            }}
          >
            <Text style={{ fontSize: 13, color: isSelected ? color : '#8A95A3', fontWeight: isSelected ? '600' : '400' }}>
              {opt}
            </Text>
          </Pressable>
        );
...
function QuestionnaireSection({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <View style={q.section}>
      <Text style={q.sectionLabel}>{label}</Text>
      {children}
    </View>
  );
}
...
function QRow({ label, value }: { label: string; value?: string | number | string[] }) {
  ...
  return (
    <View style={q.row}>
      ...
      <Text style={q.rowValue}>{display}</Text>
    </View>
  );
}
        ...
        {Array.from({ length: max }).map((_, i) => (
          <View
            ...
            style={{
              ...
              height: 6,
              borderRadius: 2,
              backgroundColor: i < value ? '#F5A623' : '#2A3347',
            }}
          />
        ))}
      </View>
  ...
  if (!intake) {
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 40 }}>
        ...
        <Text style={{ color: '#4A5568', fontSize: 16, fontFamily: FONT_HEADING, marginTop: 16, textAlign: 'center' }}>
          No questionnaire submitted yet
        </Text>
        <Text style={{ color: '#4A5568', fontSize: 14, fontFamily: FONT_BODY, marginTop: 8, textAlign: 'center' }}>
          This member has not completed the intake form.
        </Text>
      </View>
    );
  }
    ...
    <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        <View style={{ marginBottom: 20 }}>
          ...
          {(plan.currentWeight || plan.goalWeight) ? (
            <View style={[pv.darkCard, { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }]}>
              <View>
                <Text style={pv.miniLabel}>CURRENT</Text>
              ...
              <View style={{ alignItems: 'flex-end' }}>
                <Text style={[pv.miniLabel, { color: '#5B9BD5' }]}>GOAL</Text>
      ...
      {plan.whyStatement ? (
        <View style={{ marginBottom: 20 }}>
          ...
          <View style={[pv.darkCard, { borderLeftWidth: 3, borderLeftColor: '#F5A623', marginBottom: 10 }]}>
            <Text style={[pv.miniLabel, { color: '#F5A623', marginBottom: 8 }]}>
          ...
          <View style={pv.darkCard}>
            <View style={{ marginBottom: 14 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                <Text style={{ fontSize: 13, fontWeight: '600', color: '#8A95A3', letterSpacing: 0.4 }}>Readiness for Change</Text>
            ...
            <View style={{ marginBottom: 14 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                <Text style={{ fontSize: 13, fontWeight: '600', color: '#8A95A3', letterSpacing: 0.4 }}>Motivation</Text>
            ...
            {plan.gymConfidence !== undefined && plan.gymConfidence > 0 ? (
              <View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                  <Text style={{ fontSize: 13, fontWeight: '600', color: '#8A95A3', letterSpacing: 0.4 }}>Gym Confidence</Text>
      ...
      {plan.phases && plan.phases.length > 0 && (
        <View style={{ marginBottom: 20 }}>
          ...
          {plan.phases.map((phase, i) => (
            <View key={phase.id} style={[pv.darkCard, { borderLeftWidth: 3, borderLeftColor: phaseColors[i], marginBottom: 10 }]}>
...
function PlanEditor({ plan, updatePlan }: { plan: MemberPlanData; updatePlan: (updates: Partial<MemberPlanData>) => void }) {
  ...
  const currentPlan = plan[currentWeekKey] as DayPlan[];
    ...
    <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 60 }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        <Image
          ...
          resizeMode="contain"
        />
        <Text style={{ fontSize: 13, color: '#F5A623', fontWeight: '700', letterSpacing: 0.5, fontFamily: FONT_HEADING }}>
          Fitness Plan Editor
        </Text>
      </View>
      ...
      <SectionEditor title="Hero / Identity" color="#5B9BD5" collapsed={!!collapsed.hero} onToggle={() => toggle('hero')}>
        ...
        <EditField label="Identity Tag" value={plan.identityTag} onChangeText={(v) => updatePlan({ identityTag: v })} hint="e.g. Ready to train seriously" placeholder="Ready to train" />
        <EditField label="Referred By" value={plan.referredBy || ''} onChangeText={(v) => updatePlan({ referredBy: v })} placeholder="Coach name or referrer" />
        <EditField label="Plan Subtitle" value={plan.subtitle || plan.planSubtitle || ''} onChangeText={(v) => updatePlan({ subtitle: v, planSubtitle: v })} multiline hint="The tagline shown below the member's name" />
      </SectionEditor>
      ...
      <SectionEditor title="Weekly Plan" color="#5B9BD5" collapsed={!!collapsed.weekly} onToggle={() => toggle('weekly')}>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 16 }}>
          {([2, 3, 4] as const).map((n) => (
            <Pressable
              key={n}
              onPress={() => updatePlan({ sessionsPerWeek: n })}
              style={{
            ...
            >
              ...
              <Text style={{ fontSize: 10, color: '#8A95A3', marginTop: 2 }}>days/wk</Text>
            </Pressable>
          ))}
        </View>

        <EditNumberField label="Contract Length" value={plan.contractLengthMonths} onChange={(v) => updatePlan({ contractLengthMonths: v, contractMonths: v })} suffix="months" />

        {/* Day-by-day editor */}
        <Text style={[es.fieldLabel, { marginBottom: 8, marginTop: 8 }]}>Day Schedule ({plan.sessionsPerWeek} sessions/week)</Text>
        {currentPlan.map((day, i) => (
          <View key={i} style={{ marginBottom: 10, backgroundColor: '#161B25', borderRadius: 10, padding: 10, borderWidth: 1, borderColor: '#2A3347' }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              <Text style={{ fontSize: 13, fontWeight: '700', color: '#F0F4F8', width: 80 }}>{day.day}</Text>
                ...
                {dayTypeOptions.map((opt) => (
                  <Pressable
                    ...
                    onPress={() => {
                      ...
                      updatePlan({ [currentWeekKey]: updated });
                    }}
                    style={{
                      flex: 1, paddingVertical: 4, borderRadius: 6, alignItems: 'center',
                      backgroundColor: day.type === opt.value ? (typeColors[opt.value]?.bg || '#161B25') : 'transparent',
                      borderWidth: 1, borderColor: day.type === opt.value ? (typeColors[opt.value]?.border || '#2A3347') : 'transparent',
                    }}
                  >
                    <Text style={{ fontSize: 9, fontWeight: '600', color: day.type === opt.value ? (typeColors[opt.value]?.text || '#8A95A3') : '#4A5568' }}>
      ...
      {/* ── Coaching Phases ── */}
      <SectionEditor title="Coaching Evolution Phases" color="#F5A623" collapsed={!!collapsed.phases} onToggle={() => toggle('phases')}>
        {(plan.phases || []).map((phase, i) => (
          <View key={i} style={{ marginBottom: 14, backgroundColor: '#161B25', borderRadius: 10, padding: 12, borderWidth: 1, borderColor: phaseColors[i] + '30' }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <View style={{ width: 24, height: 24, borderRadius: 12, backgroundColor: phaseColors[i] + '20', alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ fontSize: 12, fontWeight: '700', color: phaseColors[i] }}>{i + 1}</Text>
              </View>
              <TextInput
                style={[es.input, { flex: 1 }]}
                value={phase.name}
                onChangeText={(v) => {
                  ...
                  updatePlan({ phases: updated });
                }}
                placeholder="Phase name"
                placeholderTextColor="#3A4255"
              />
              <TextInput
                style={[es.input, { width: 60 }]}
                value={String(phase.weeks)}
                onChangeText={(v) => {
                  const updated = [...plan.phases];
                  updated[i] = { ...updated[i], weeks: parseInt(v) || 0 };
                  updatePlan({ phases: updated });
                }}
                ...
                placeholderTextColor="#3A4255"
              />
              <Text style={{ fontSize: 11, color: '#8A95A3' }}>wks</Text>
            </View>
            <TextInput
              style={[es.input, { minHeight: 60, textAlignVertical: 'top' }]}
            ...
            {plan.phases.length > 1 && (
              <Pressable
                onPress={() => updatePlan({ phases: plan.phases.filter((_, j) => j !== i) })}
        ...
        <Pressable
          onPress={() => {
            const newPhase: Phase = { id: plan.phases.length + 1, name: '', weeks: 12, description: '' };
      ...
      <SectionEditor title="Investment & Pricing" color="#F5A623" collapsed={!!collapsed.investment} onToggle={() => toggle('investment')}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          ...
          <Pressable
            onPress={() => updatePlan({ showInvestment: !plan.showInvestment })}
            style={{
          ...
          >
            <Text style={{ fontSize: 13, fontWeight: '600', color: plan.showInvestment ? '#6EBB7A' : '#8A95A3' }}>
              {plan.showInvestment ? 'Visible' : 'Hidden'}
            </Text>
          </Pressable>
        </View>
        <EditNumberField label="Hourly Rate" value={plan.hourlyRate} onChange={(v) => updatePlan({ hourlyRate: v })} prefix="$" suffix="/hr" hint="Your coaching hourly rate" />
        <EditNumberField label="Monthly Price" value={plan.monthlyPrice} onChange={(v) => updatePlan({ monthlyPrice: v })} prefix="$" suffix="/mo" hint="What the member pays per month" />
        <EditNumberField label="Per Session Price" value={plan.perSessionPrice} onChange={(v) => updatePlan({ perSessionPrice: v })} prefix="$" suffix="/session" />
        <EditNumberField label="Pay in Full Total" value={plan.payInFullPrice} onChange={(v) => updatePlan({ payInFullPrice: v })} prefix="$" hint="Total price if member pays upfront" />
        <EditNumberField label="Pay in Full Discount" value={plan.payInFullDiscount} onChange={(v) => updatePlan({ payInFullDiscount: v })} suffix="%" hint="Discount percentage for pay-in-full" />
      </SectionEditor>
          ...
          {(['draft', 'presented', 'active'] as const).map((st) => (
            <Pressable
              ...
              style={{
                ...
                borderWidth: 1,
                borderColor: plan.status === st ? (st === 'active' ? 'rgba(110,187,122,0.3)' : st === 'presented' ? 'rgba(91,155,213,0.3)' : '#2A3347') : '#2A3347',
              }}
            >
              <Text style={{
                fontSize: 13, fontWeight: '600', textTransform: 'capitalize',
                color: plan.status === st ? (st === 'active' ? '#6EBB7A' : st === 'presented' ? '#5B9BD5' : '#8A95A3') : '#4A5568',
              }}>{st}</Text>
            </Pressable>
          ))}
        </View>
        <Text style={[es.fieldHint, { marginTop: 8 }]}>
          Draft = only you can see it. Presented = member can view. Active = plan is live.
        </Text>
      </SectionEditor>
    </ScrollView>
  );
...
// ─── Plan Viewer (Coach — wraps editor + preview) ───────────────────────────
...
function PlanViewer({ plan, memberName, memberId, onPlanUpdate }: {
  ...
  if (!localPlan) {
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 40 }}>
        <Icon name="document" size={48} color="#2A3347" />
        <Text style={{ color: '#4A5568', fontSize: 16, fontFamily: FONT_HEADING, marginTop: 16, textAlign: 'center' }}>
          No fitness plan created yet
        </Text>
        <Text style={{ color: '#4A5568', fontSize: 14, fontFamily: FONT_BODY, marginTop: 8, textAlign: 'center' }}>
          Create a personalized fitness plan for {memberName}.
        </Text>
        <TouchableOpacity
          style={{
            marginTop: 24, backgroundColor: '#F5A623', borderRadius: 12,
  ...
  return (
    <View style={{ flex: 1 }}>
      {/* Toolbar: View Toggle + Save + Share */}
      <View style={tb.toolbar}>
        {/* View Toggle */}
        <View style={tb.toggleContainer}>
          <Pressable
            onPress={() => setViewMode('edit')}
            style={[tb.toggleBtn, viewMode === 'edit' && tb.toggleBtnActive]}
          >
          ...
          <Pressable
            ...
            style={[tb.toggleBtn, viewMode === 'preview' && tb.toggleBtnActive]}
          >
            <Icon name="person" size={14} color={viewMode === 'preview' ? '#5B9BD5' : '#4A5568'} />
            <Text style={[tb.toggleText, viewMode === 'preview' && { color: '#5B9BD5' }]}>Preview</Text>
          </Pressable>
        </View>
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          {/* Share */}
          <Pressable onPress={handleShareLink} style={tb.actionBtn}>
            <Icon name="share" size={16} color="#5B9BD5" />
          </Pressable>
...
export default function MemberPlanScreen() {
  ...
  async function loadData() {
    ...
    try {
      ...
      console.error('[MemberPlan] member profile error:', e.message);
    }
    ...
    try {
      ...
      console.error('[MemberPlan] intake submission error:', e.message);
    }
    ...
    try {
      ...
      console.error('[MemberPlan] plan load error:', e.message);
    }
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.tabBar}>
        <Pressable
          style={[s.tab, activeTab === 'questionnaire' && s.tabActive]}
      ...
      {activeTab === 'questionnaire' ? (
        ...
        <PlanViewer
          ...
          memberId={memberId}
          onPlanUpdate={(updated) => setPlanData(updated)}
        />
      )}
    </View>
  );
