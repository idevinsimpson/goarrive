...
- **A partial cycle is invalidated on loss, on any unobserved frame, and across a stream gap.** Leaving `locked` for any reason calls `interrupt()`. So does a single frame with no trusted pose during a partial cycle, and so does a frame gap over 250 ms or an out-of-order frame (`movement-fail-closed.test.ts`). That discards the rep in progress, and the member must be seen standing again (`session.ts`). Tests: "interrupt() mid-rep discards the rep", "lost tracking at the bottom voids that rep …", "while lost, nothing is counted …". The mutation check confirmed these tests go red if `interrupt()` becomes a no-op.
...
6. **Detections are never merged.** Every detection is treated as a distinct person, and overlapping detections read as a crowd, which pauses counting. If an engine ever reports one body twice, counting pauses rather than trusting it. Before #475 review `5825938854` detections with IoU ≥ 0.7 were merged; that allowed two distinct overlapping people to become one trusted subject.
...
9. **It needs about 10 fps or more, and at least 5 fps to count at all.**
   - Down and standing each need 100 ms of consecutive samples, so at low frame rates fast reps can be missed.
   - Frames more than 250 ms apart (`maxFrameGapMs`) count as an interrupted stream: the rep in progress is voided and the member is re-acquired. Below about 4 fps, nothing is ever counted.
   - A frame whose timestamp is not after the previous one is rejected and voids the rep in progress.
   - Emulated-GPU browsers ran at about 1 fps; use `?delegate=cpu` there.
...
12. **A rep that is not fully observed is discarded, never completed.** This covers lost tracking, a single frame with no trusted pose partway through the rep, and a stream gap. On a real camera, one dropped detection at the bottom of a squat voids that rep, so the member might feel a rep they did was not counted. That is the intended trade: a missed rep is better than an invented one. Real-device dropout rates are not measured.
