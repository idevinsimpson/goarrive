...
| **E3** | Challenge templates, community challenges, check-ins | M-U4 — **the product at FitLife** | Maia — **WIP** `feat/wsf-e3-challenges` @ `858feb2` (17:00Z checkpoint: three callables + three callable tests. **PM review 17:10Z:** criteria 1–9 PRESENT, 10 PARTIAL; sharded counter and idempotent check-in correct; callable suite 45/45 and the 50-concurrent burst EMULATOR VERIFIED by the PM's own run. Five gaps dispatched 17:12Z: unthrottled public pulse, `requiresCode` not enforced, membership/status checks outside the transaction, id validation → `internal`, sequential shard reads. Fixes landed as `4f6cc54` (17:46Z). **GATE 1 GREEN @ `abcc66b` 18:08:50Z — EMULATOR VERIFIED:** callable suite 54/54 (incl. the 50-concurrent burst) + 6 e2e, via the self-reporting runner. The one red run before it (17:47Z) was the test harness, not the product: firebase-admin's GCE metadata probe on the first Firestore call per jest worker; fixed with `METADATA_SERVER_DETECTION=none` in the gate. **Review of `4f6cc54` (18:15Z):** gaps 1, 2, 3, 5 PASS; 4 PARTIAL — the new id floor (16 chars) would reject hand-made FitLife ids; relax to `{1,128}`. Follow-ups queued for the next callable turn: trust the rightmost `x-forwarded-for` entry and make the limiter counter atomic, cache before limiter, code gate after the idempotent return, two more tests, plus a `beforeAll` warm-up in the three first-touch suites. **Turn A `12f4869` (18:35Z): gate ✅ GREEN 18:37Z** — id floor relaxed to `{1,128}`, cache-first pulse, atomic rightmost-ip limiter, code gate after the idempotent return, two more tests, first-touch warm-up. Turn B (member challenge screen, `seed-fitlife.mjs`, E3 e2e in `gate1.sh`) dispatched 18:41Z.) |
