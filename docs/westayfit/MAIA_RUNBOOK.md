...
- **Silent stall after a detached wait (2026-09-12, 01:33 EDT →).** In the E4-A1-R1 resume turn Maia wrote "Edit verified. Waiting for the detached rebuild + E2E (~5–7 min)" and then posted nothing for at least 59 minutes: no heartbeat, no 45-minute deadline notice, no detached-job result post, no error. This is a different signature from 2026-09-05 (heartbeats kept coming there). Undiagnosed; the thread alone cannot tell a dead turn from a wait with suppressed heartbeats. Only the box can (rule 13 probe, then "Recovering her from the box"). If the detached job was started with a hand-rolled `systemd-run` unit rather than `scripts/westayfit/run-detached.sh`, its result is never posted to the thread — check the unit's log on the box before assuming the job failed.
