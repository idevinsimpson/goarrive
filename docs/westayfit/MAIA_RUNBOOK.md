...
21. **Never `pkill -f` from a bot shell with a pattern that can appear in the invoking command line; terminate by exact PID, graceful signal first.** `pkill -f` matches full command lines, and the `bash -c` line running the pkill contains the pattern text, so it kills its own shell (seen 2026-09-12 14:10 EDT, exit 144, in the E4-A1-R4 run) and can match other workflows' emulators on the same box. The existing rule stands: never `pkill node` or `bot.js`.

22. **Never read a task output file wholesale from a turn; bound every read.** `cat` or `tail -f` of a growing jest/emulator log pulls the whole file through the tool and pressures memory, which is the 2026-09-05 wedge shape; on 2026-09-12 the stalls (two git reads hitting the 10-minute command timeout, then a 51-minute turn) followed exactly such reads. Use `tail -c 20000` or `grep -c`, and read a detached job's status file, not its log.
...
- **45-minute turn deadline vs long packages (2026-09-11).** Maia's E4-A1 turn was cut at the turn deadline mid-tool-call (21:17 EDT) and had to be re-dispatched. Packages that install deps, boot emulators and run suites do not fit one turn; either raise the deadline for code packages or dispatch them as ≤ 40-minute steps with an explicit resume checkpoint. Second occurrence 2026-09-12 00:57 EDT: the E4-A1-R1 turn was cut the same way mid-edit and resumed by ChatGPT at the tool boundary; the resume depended on an uncommitted (and, per the bot's narration, later stashed) working tree. Preferred mitigation for code packages: a local WIP commit on the package branch before each long step (a local commit is neither a new branch nor a push, so it stays inside the package rules), plus ≤ 40-minute steps.
- **Silent stall after a detached wait (2026-09-12, 01:33 EDT →) — the turn died, the job did not.** In the E4-A1-R1 resume turn Maia wrote "Edit verified. Waiting for the detached rebuild + E2E (~5–7 min)" and then posted nothing for 70 minutes: no heartbeat, no 45-minute deadline notice, no 90-minute cap notice, no error. The detached unit (`wsf-e4a1-e2e-run.service`, a hand-rolled `systemd-run` unit, not `run-detached.sh`) had finished successfully at 01:34:17 EDT, 57 s after that post, and wrote its status file; nobody saw it because the turn never came back and a hand-rolled unit posts nothing to the thread. The next dispatch at 02:43 EDT (after the cap) was acknowledged in 3 s and answered in 2 minutes, so the service was healthy: a dead turn, not a wedge. Different signature from 2026-09-05 (heartbeats kept coming there). Still unknown: why the turn died silently and why no cap notice was posted. Rules that follow: long jobs go through `scripts/westayfit/run-detached.sh` so the result lands in the thread as text whether or not the turn survives; a director seeing this signature waits out the cap, then re-dispatches with "inspect the unit, do not restart the suite" (as ChatGPT did).
- **Non-audio uploads to Slack (2026-09-12).** E4-A1-R2 required the patch, screenshots and evidence note uploaded into the thread; Maia's report named `/tmp` paths and attached nothing. Her bot uploads files named by `[ATTACH: /path]` markers in the reply (Maia located the mechanism in `bot.js` at 06:03 EDT 09-12; first real use pending). A dispatch that needs reviewable artifacts should ask for the upload with the SHA-256 printed alongside and say "post without asking" (rule 19); a push of the package branch with no PR remains the director's or Devin's call.
- **Auto-narration paraphrases commands (2026-09-12).** The bot's tool-narration lines are generated descriptions, not quotes: "reviewing the deployment log" described a local jest log, and "executing the eight-file plan" described an `echo`. Read the command line and Maia's own text; never grade a turn by its narration.
- **Gates in a sandbox behind an allowlisting proxy (PM container, 2026-09-12).** The Expo CLI crashes at start/export when its doctor lookup to `api.expo.dev` gets a non-JSON 403; `EXPO_OFFLINE=1 EXPO_NO_DEPENDENCY_VALIDATION=1` keeps it local-only. The repo's Playwright wants a newer headless shell than a pre-installed browser set may have; `WSF_PLAYWRIGHT_CHROMIUM=<chrome binary>` (already supported by `playwright.config.ts`) fixes that. Until `app/signin.tsx:111` is fixed, `ts:check` passes only when `apps/westayfit/.expo/types` is absent (fresh checkout) — any `expo start`/`export` generates it. The dev-server browser path serves the new dynamic routes without hosting rewrites but does not carry the static export's injected `robots` meta, so specs asserting it (mu2, e2 join) need the real static path.
- **Wedge after the R4 approval (2026-09-12, 14:10 EDT →).** Sequence: broad self-matching `pkill -f` (exit 144) at 14:10, two git reads timing out at 10m02s, the implementation turn cut at 51m26s (14:33) with a queued director message discarded at a session reset, a 14:39 turn that acked in 3 s and then produced only a "24m41s working…" heartbeat, and a 15:04 mention with no acknowledgment for over an hour. Same recovery path as 2026-09-05: nothing from Slack; the box procedure below. The R4 edits Maia reported (`src/firebase.ts`, `functions-westayfit/src/index.ts`, the contribute route, the e2e spec, `tests/firebase-projectId-gate.test.ts`) live only in `~/dev-goarrive-wsf-e4a1` and survive a service restart; check them before re-running anything.
