...
- **Non-audio uploads to Slack (2026-09-12).** E4-A1-R2 required the patch, screenshots and evidence note uploaded into the thread; Maia's report named `/tmp` paths and attached nothing. Whether her bot can upload non-audio files is unverified. Until it is, a dispatch that needs reviewable artifacts should ask for one of: a Slack file upload with the SHA-256 printed alongside (prove the tool exists first with a tiny text file); a push of the package branch with no PR (a push is the director's or Devin's call); or the patch pasted in a code block when it is small enough.
