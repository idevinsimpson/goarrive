...
`WE_STAY_FIT_NORTH_STAR_BOARD_08_GOAL_SETUP_FINAL.png` · 2560×5302 (1280×2651 @2x)
...
| --- | --- | --- |
| `page-02-move/before/BEFORE-goal-new-390x{844,640}.png` | photographs of the real `/goals/new`, captured at `02e24df` (2026-09-21) and frozen by `check-evidence-intact.mjs`. Named BEFORE because they were shot as the MOVE package's baseline; what they photograph is this route, and the route has not changed since. **Arrival only.** | `CURRENT BUILD · CAPTURED` |
...
There is no third tag, because there is no third state: this route has no
accepted-page verdict and no `after/` set.
...
| duration choices are exactly 1 week / 2 weeks / 1 month / Custom | `1 week` selected is visible on the 844 capture; the full set is on the below-the-fold panel, read from `DURATIONS` in the route |
| preset start is the current quarter-hour floor; preset end derives from that start | Below-the-fold panel, from `quarterHourFloor` / `addDuration` |
| Custom exposes both start/end | Below-the-fold panel and the lock panel |
| time zone comes from the device, stated in words, not guessed and not a hidden picker | Below-the-fold panel, quoting the route's `Times are in <zone>` line; the spec asserts no picker and no Change control exist |
| repeat policy defaults to `One contribution per member`; `Members can contribute again` is explicit | Below-the-fold panel and the lock panel. **Not captured** — this card is beneath the fold in both frames |
| `Check it over` is a card/region on the SAME form, not a separate wizard screen | Below-the-fold panel, naming all seven summary rows |
| summary reads back Community, Goal, Target, Starts, Ends, Time zone and Members policy | Same panel, listing exactly those |
| validation appears after submit, under its field; the first refused field receives focus/scroll | Below-the-fold panel; also the one thing the drawings get right, captioned on `Refused fields, drawn` |
| button state `Start this goal` → `Starting…` | Below-the-fold panel |
| mutually-exclusive server errors never stacked; refusals are human copy, no raw Firebase codes | Below-the-fold panel and the lock panel. The route holds **one** error string and maps codes through `describeServerError` |
| created: `Your goal is live` + `Send it to your members and put it on a screen.`, the summary, one primary `Open the contribute page`, secondary `Show on a big screen`, `Back to community` | Below-the-fold panel, in the route's own words. **No capture of the created screen exists**, so it is named and not drawn; `Created, drawn` beside it is the batch-b drawing, tagged as one |
...
| common states — signed out → Sign in; unverified → Verify your email; no community context → `Choose a community before starting a goal` + go to communities; validation, server unavailable, submitting and created distinct | Named in the lock panel. The no-community wording is quoted against the drawing, which says something else |
...
- It does not draw a state nobody photographed. Repeat policy, the summary,
  submitting, the created screen, signed-out, unverified, no-community and the
  server refusals have **no capture of the real route**, so they are named with
  their source and left undrawn.
...
- It does not edit, re-capture or re-encode any source PNG, and it does not
  ...
  manifest, `review-copies/`, the INDEX, Boards 00–07, `.github/`, app or backend
  code, or any test.

## Fixtures

"Alpharetta Morning Movers" on the captures, and "The Henderson Family" /
"October Push-Up Challenge" on the drawings, are synthetic and stay exactly as
captured. No real community, person or goal appears. No faces, names, quotes,
...
| Determinism | Board 08 rendered twice, identical bytes (`cmp`) |
| Canvas | content height 2651 CSS px; footer at 2585–2615, inside the canvas; every panel and every frame inside, nothing clipped |
