...
- **Product head:** `b497ce4c`. Later commits change tests and evidence only.
...
| `move-receipt` | The same flow, after Confirm | The navy page | The navy panel over Home; the outcome exit "Back to community" is kept |
...
7. **Focus on step change.** The reference focuses each step's heading. Here focus enters the sheet's first control on open and is not moved on each step.
