...
status distinct from acceptance). Proceed only after W4's candidate-level
regression on `d0477cc` and W3's pin-readiness verdict; if any required step
turns out uncovered, name that exact step and record rather than a generic
'awaiting owner'. Staging still serves `3562156`; staging is claimed only on a
