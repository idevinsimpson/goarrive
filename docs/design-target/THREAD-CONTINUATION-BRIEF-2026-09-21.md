...
(receipt `5786432078`). Staging still serves `3562156` until run 45's deploy
job and hosted build-marker verification pass; Hosting receipt, hosted-verify
and cleanup are reported separately.** Staging still serves `3562156`; staging is claimed only on a
