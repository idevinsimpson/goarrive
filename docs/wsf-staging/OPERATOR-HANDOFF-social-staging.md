...
(#365 `5798443901`). Authority: the owner's incremental staging authorization
...
| measure first | the run's own per-service transport line decides; nothing is declared shut or open from history |
| permission | `run.services.update` + `run.services.get` on the three services (Cloud Run Admin on those services, or owner / editor) |
...
| **receipt fields** | per service: `gcloud run services describe <service> --project=westayfit-staging --region=us-central1 --format='value(invokerIamDisabled)'` after the change (or the console setting) · identity · timestamp; then the verifier / hosted smoke line proving reachability and the member / non-member / name-off / activity-off results |
