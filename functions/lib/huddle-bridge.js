...
exports.marcoHuddleTurn = void 0;
...
// ─── OpenAI helper ────────────────────────────────────────────────────────────
async function getOpenAIReply(apiKey, messages) {
    var _a, _b, _c, _d, _e;
    ...
    return (_e = (_d = (_c = (_b = (_a = json.choices) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.message) === null || _c === void 0 ? void 0 : _c.content) === null || _d === void 0 ? void 0 : _d.trim()) !== null && _e !== void 0 ? _e : '(No response from Marco)';
}
...
async function loadHuddleHistory(huddleId) {
...
function buildMarcoHuddleSystemPrompt(topic, mode, assignedPosition, sharedBrain) {
    const positionNote = mode === 'advocate' && assignedPosition
...
function buildHuddleMessages(history, systemPrompt) {
...
function buildTranscriptSlackMessage(topic, history, synthesis, mode, assignedPosition) {
    const modeLabel = mode === 'advocate'
    ...
    const transcriptBlock = rounds
        .map((r, i) => {
        const lines = [`:speech_balloon: *Round ${i + 1}*`];
        if (r.maia)
            lines.push(`*Maia:* ${r.maia}`);
        if (r.marco)
            lines.push(`*Marco:* ${r.marco}`);
        return lines.join('\n');
    })
        .join('\n\n');
...
async function generateSynthesis(openaiKey, topic, history) {
...
exports.marcoHuddleTurn = (0, firestore_1.onDocumentCreated)({
    ...
    const { huddle_id, turn_index, topic, assigned_position, mode, slack_channel, slack_thread_ts, slack_user_id, } = data;
    ...
    try {
        ...
        const systemPrompt = buildMarcoHuddleSystemPrompt(topic, mode, assigned_position, sharedBrain);
        ...
        if (isLastTurn) {
            ...
            const fullHistory = [
                ...history,
                Object.assign(Object.assign({}, marcoTurnDoc), { turn_index: turn_index + 1 }),
            ];
            const synthesis = await generateSynthesis(openaiKey, topic, fullHistory);
            const transcriptMessage = buildTranscriptSlackMessage(topic, fullHistory, synthesis, mode, assigned_position);
        ...
        try {
            await postSlackMessage(botToken, slack_channel, `⚠️ Marco encountered an error on huddle turn ${turn_index + 1}: ${(_a = err.message) !== null && _a !== void 0 ? _a : 'Unknown error'}. The huddle may be incomplete.`, slack_thread_ts);
        }
        catch (slackErr) {
...
//# sourceMappingURL=huddle-bridge.js.map