...
import {
  MaiaBridgeError,
  enforceMaiaHonesty,
  formatHuddleTranscript,
  getMaiaBrainReply,
} from '../slack';

describe('getMaiaBrainReply — strict failure modes', () => {
  test('throws MaiaBridgeError(missing_key) when key is empty string', async () => {
    await expect(getMaiaBrainReply('', 'analysis', 'question', 'user msg')).rejects.toBeInstanceOf(
      MaiaBridgeError
    );
    await expect(getMaiaBrainReply('', 'a', 'q', 'm')).rejects.toMatchObject({
      stage: 'missing_key',
    });
  });
...
describe('formatHuddleTranscript', () => {
  test('returns empty string when huddled=false (degraded — no real Maia)', () => {
    const out = formatHuddleTranscript({
      finalReply: 'whatever',
      huddled: false,
...
describe('enforceMaiaHonesty', () => {
  ...
  test('scrubs "we huddled" phrases when context not loaded', () => {
    const out = enforceMaiaHonesty('We huddled with Maia and decided X.', false);
