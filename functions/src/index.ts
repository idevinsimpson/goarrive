...
async function handleCheckoutSessionCompleted(
  ...
  const { intentId, planId, snapshotId, memberId, coachId: _coachId, submissionId, funnelCheckout, discountCodeUsed } = session.metadata ?? {};
  ...
  if (funnelCheckout === 'true' && submissionId) {
    const funnelCoachId = session.metadata?.coachId;
    const subRef = db.collection('onboarding_submissions').doc(submissionId);
    const customerId = typeof session.customer === 'string' ? session.customer : (session.customer as any)?.id;
    const funnelSubscriptionId = typeof session.subscription === 'string' ? session.subscription : (session.subscription as any)?.id;

    // 1. PR-H bookkeeping: mark submission paid + record Stripe IDs
    await subRef.update({
      status: 'paid',
      stripeCustomerId: customerId ?? null,
      stripeSubscriptionId: funnelSubscriptionId ?? null,
    ...
    let funnelMemberId: string | null = null;
    try {
      const submSnap = await subRef.get();
      const subm = submSnap.data() || {};
      const submEmail = (subm.email as string | undefined)?.trim().toLowerCase();
      const submFirst = (subm.firstName as string | undefined) ?? '';
      const submLast = (subm.lastName as string | undefined) ?? '';
      ...
      await enrollSubscriber({
        ...
        stripeSubscriptionId: funnelSubscriptionId ?? '',
  ...
  async (request) => {
    ...
    if (guidancePhase !== 'self_guided') {
      ...
      try {
        ...
        const gcalAccounts: any[] = coachDataForGcal?.gcalConflictAccounts || [];
        const accountsWithCalendars = gcalAccounts.filter((a: any) =>
          (a.selectedCalendarIds || []).length > 0 && !a.tokenExpired
        );
        if (accountsWithCalendars.length > 0) {
          ...
          for (const account of accountsWithCalendars) {
            try {
              ...
              const calendarApi = google.calendar({ version: 'v3', auth: oauth2ClientForCheck });
              const freebusyRes = await calendarApi.freebusy.query({
                requestBody: {
                  timeMin: startISO,
                  timeMax: endISO,
                  timeZone: timezone || 'UTC',
                  items: account.selectedCalendarIds.map((id: string) => ({ id })),
                },
              });
              const calendarsData = freebusyRes.data.calendars || {};
              for (const calId of account.selectedCalendarIds) {
                const busy = calendarsData[calId]?.busy || [];
                if (busy.length > 0) {
    ...
    if (slotAction === 'reschedule_instance') {
      ...
      if (guidancePhase !== 'self_guided') {
        ...
        for (const cDoc of conflictSnap.docs) {
          if (cDoc.id === instanceId) continue;
          const c = cDoc.data();
          const cPhase = c.guidancePhase;
          if (cPhase === 'self_guided') continue;
          const [ch, cm] = ((c.scheduledStartTime || c.startTime || '00:00') as string).split(':').map(Number);
          const cStart = ch * 60 + cm;
          const cEnd = cStart + (c.durationMinutes || 30);
          if (rStart < cEnd && rEnd > cStart) {
            throw new HttpsError('already-exists', `Conflict: ${c.memberName || 'another member'} has a session at ${c.scheduledStartTime || c.startTime} on ${newDate}. Choose a different time.`);
          }
        }
      }
  ...
  async (req, res) => {
    ...
    switch (eventType) {
      ...
      case 'recording.completed': {
        ...
        const transcriptFiles = recordingFiles.filter((f: any) => f.file_type === 'TRANSCRIPT' || f.recording_type === 'audio_transcript');
        const transcripts = transcriptFiles.map((f: any) => ({
          downloadUrl: f.download_url,
          fileType: f.file_type,
          status: f.status,
        }));
        if (occurrenceId) {
          ...
          if (transcripts.length > 0) recUpdate.transcripts = transcripts;
        ...
        case 'meeting.rtms_started': {
          ...
          if (!dispatched) {
            ...
            fetch(workerUrl, {
              ...
              console.error('[zoomRtmsWebhook] Fallback dispatch also failed:', err?.message || err);
            });
          }
          break;
        }
...
async function dispatchViaCloudTasks(args: {
  workerUrl: string;
  payload: { meetingId: string; streamId: string; meetingUuid?: string };
}): Promise<boolean> {
  const project = process.env.GCLOUD_PROJECT || process.env.GCP_PROJECT;
  ...
  try {
    ...
    if (code === 5 || code === 7) {
      ...
      console.error('[dispatchViaCloudTasks] Enqueue failed:', err?.message || err);
    }
    return false;
  }
}
...
export const coachIcalFeed = onRequest(
  ...
  async (req, res) => {
    ...
    const storedToken = coachData.icalToken as string | undefined;
    if (storedToken) {
...
export const requestSkipInstance = onCall(
  ...
  async (request) => {
    ...
    if (pendingSnap.size >= 3) {
      throw new HttpsError('resource-exhausted', 'You already have 3 pending skip requests. Please wait for your coach to review them before requesting more.');
    }
    ...
    try {
      ...
      if (coachSnap.exists) {
        ...
        const autoCategories = coachData.autoApproveSkipCategories as string[] | undefined;
        const autoLeadDays = coachData.autoApproveSkipLeadDays as number | undefined;
        ...
        if (!autoApproved && typeof autoLeadDays === 'number' && autoLeadDays > 0) {
          const [year, month, day] = (inst.scheduledDate as string).split('-').map(Number);
          const [h, m] = ((inst.scheduledStartTime || inst.startTime || '00:00') as string).split(':').map(Number);
          const sessionTime = new Date(year, month - 1, day, h, m);
          const hoursUntil = (sessionTime.getTime() - Date.now()) / (1000 * 60 * 60);
          if (hoursUntil >= autoLeadDays * 24) {
            autoApproved = true;
    ...
    try {
      ...
      const notifPayload = {
        messageType: 'skip_request_received' as const,
        recipient: {
          uid: inst.coachId,
          email: coachData.email,
      ...
      await sendNotification({ ...notifPayload, channel: 'push' });
      // Send email notification if coach has email
      if (coachData.email) {
        await sendNotification({
          ...notifPayload,
          channel: 'email',
          htmlBody: `<p><strong>${inst.memberName || 'A member'}</strong> requested to skip their <strong>${inst.scheduledDate}</strong> session.</p><p><strong>Category:</strong> ${resolvedCategory}</p>${reason ? `<p><strong>Reason:</strong> ${reason}</p>` : ''}<p>Open the app to approve or deny this request.</p>`,
        });
      }
    } catch (err: any) {
...
export const refreshRecordingUrl = onCall(
  ...
  async (request) => {
    ...
    if (!isCoach && !isAdmin) {
      throw new HttpsError('permission-denied', 'Only coaches and admins can access recordings');
    }
    ...
    if (isCoach && !isAdmin && inst.coachId !== callerUid) {
      throw new HttpsError('permission-denied', 'You can only access recordings for your own sessions');
    }

    const zoomMeetingId = inst.zoomMeetingId;
    if (!zoomMeetingId) {
      throw new HttpsError('failed-precondition', 'No Zoom meeting associated with this session');
    }
    ...
    try {
      ...
      const realProvider = provider as any;
      ...
      const freshPlayUrl = data.recording_files?.find(f => f.file_type === 'MP4')?.play_url;
      const freshDownloadUrl = data.recording_files?.find(f => f.file_type === 'MP4')?.download_url;
      const recordingUrl = freshShareUrl || freshPlayUrl || inst.zoomRecordingUrl;

      // Update the instance with fresh URLs
      const updateData: Record<string, any> = {
        updatedAt: FieldValue.serverTimestamp(),
        recordingRefreshedAt: FieldValue.serverTimestamp(),
      };
      if (recordingUrl) updateData.zoomRecordingUrl = recordingUrl;
      ...
      throw new HttpsError('internal', `Failed to refresh recording URL: ${err.message}`);
    }
  }
);
...
export const regenerateIcalToken = onCall(
  ...
  async (request) => {
    ...
    if (!coachSnap.exists) throw new HttpsError('permission-denied', 'Not a coach');
    ...
    await coachRef.update({
      icalToken: newToken,
      icalTokenUpdatedAt: FieldValue.serverTimestamp(),
      updatedAt: FieldValue.serverTimestamp(),
    });
...
export const migrateIcalTokens = onCall(
  { region: 'us-central1' },
  async (request) => {
    const callerUid = request.auth?.uid;
    if (!callerUid) throw new HttpsError('unauthenticated', 'Must be signed in');
    ...
    const adminSnap = await db.collection('admins').doc(callerUid).get();
    if (!adminSnap.exists) throw new HttpsError('permission-denied', 'Only admins can run migrations');
...
function getGoogleOAuth2Client(redirectUri?: string) {
  const uri = redirectUri ?? `https://us-central1-goarrive.cloudfunctions.net/googleCalendarCallback`;
...
export const initGoogleCalendarAuth = onCall(
  ...
  async (request) => {
    ...
    if (!coachDoc.exists) throw new HttpsError('permission-denied', 'Coach only');
...
export const googleCalendarCallback = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      const coachId = req.query.state as string;
      if (!code || !coachId) {
      ...
      if (tokens.id_token) {
        try {
          ...
          if (payload.email) googleCalendarEmail = payload.email;
...
export const syncToGoogleCalendar = onCall(
  ...
  async (request) => {
    ...
    if (!refreshToken) {
      throw new HttpsError('failed-precondition', 'Google Calendar not connected. Please connect first in Account settings.');
    }
 ...
 */
export const gcalConflictCallback = onRequest(
  { secrets: [googleClientId, googleClientSecret] },
  async (req, res) => {
    try {
      const code = req.query.code as string;
      ...
      const coachData = coachDoc.data()!;
      const existingAccounts: any[] = coachData.gcalConflictAccounts || [];

      // Replace if already exists (re-auth), otherwise append
      const filtered = existingAccounts.filter((a: any) => a.accountId !== accountId);
      filtered.push({
        accountId,
        email,
        refreshToken: tokens.refresh_token,
...
export const listGcalConflictCalendars = onCall(
  ...
  async (request) => {
    ...
    if (!coachDoc.exists) throw new HttpsError('permission-denied', 'Coach only');
    const coachData = coachDoc.data()!;
    const accounts: any[] = coachData.gcalConflictAccounts || [];
    const account = accounts.find((a: any) => a.accountId === accountId);
    if (!account) throw new HttpsError('not-found', 'Conflict account not found');
...
export const updateGcalConflictCalendars = onCall(
  ...
  async (request) => {
    const callerUid = request.auth?.uid;
    if (!callerUid) throw new HttpsError('unauthenticated', 'Must be signed in');
    const { accountId, selectedCalendarIds } = request.data as {
      accountId: string;
    ...
    const coachData = coachDoc.data()!;
    const accounts: any[] = coachData.gcalConflictAccounts || [];
    const idx = accounts.findIndex((a: any) => a.accountId === accountId);
    if (idx === -1) throw new HttpsError('not-found', 'Conflict account not found');
...
export const removeGcalConflictAccount = onCall(
  ...
  async (request) => {
    ...
    const coachData = coachDoc.data()!;
    const accounts: any[] = coachData.gcalConflictAccounts || [];
    const filtered = accounts.filter((a: any) => a.accountId !== accountId);
...
export const checkGcalConflicts = onCall(
  ...
  async (request) => {
    const callerUid = request.auth?.uid;
    if (!callerUid) throw new HttpsError('unauthenticated', 'Must be signed in');
    const { startDateTime, endDateTime, timezone } = request.data as {
      startDateTime: string; // ISO 8601
      endDateTime: string;   // ISO 8601
      timezone?: string;
    };
    if (!startDateTime || !endDateTime) {
      throw new HttpsError('invalid-argument', 'startDateTime and endDateTime required');
    }
    ...
    const conflictingEvents: { calendarEmail: string; summary: string; start: string; end: string }[] = [];

    for (const account of accounts) {
      const selectedIds: string[] = account.selectedCalendarIds || [];
      if (selectedIds.length === 0) continue; // skip if no calendars selected
      ...
      try {
        ...
        const isTokenError = err.message?.includes('invalid_grant') || err.message?.includes('Token has been expired or revoked');
        if (isTokenError) {
          try {
            const coachDocRef = db.collection('coaches').doc(callerUid);
            const freshSnap = await coachDocRef.get();
            const freshAccounts: any[] = freshSnap.data()?.gcalConflictAccounts || [];
            const updated = freshAccounts.map((a: any) =>
              a.accountId === account.accountId ? { ...a, tokenExpired: true } : a
            );
...
export const generateMovementGif = onDocumentUpdated(
  ...
  async (event) => {
    ...
    const beforeFailed = before.gifGenerationFailed as boolean | undefined;
    const afterFailed = after.gifGenerationFailed as boolean | undefined;
    if (!beforeFailed && afterFailed) return;
    ...
    try {
      ...
      if (ffmpegAvailable) {
        ...
        try {
          ...
          execSync(
            `ffmpeg -y -i "${videoUrl}" -vframes 1 -vf "scale=240:300:force_original_aspect_ratio=increase,crop=240:300" "${outputPath}"`,
      ...
      await event.data!.after.ref.update({
        ...
        gifGenerationFailed: FieldValue.delete(),
        gifGenerationFailedAt: FieldValue.delete(),
        gifGenerationError: FieldValue.delete(),
      });
      ...
      try {
        await event.data!.after.ref.update({
          ...
          gifGenerationFailedAt: Timestamp.now(),
 ...
 */
export const retryFailedGifGeneration = onSchedule(
...
No markdown, no explanation.`;
    ...
    try {
      ...
      throw new HttpsError('internal', 'Failed to analyze movement reps');
    }
  },
);
    ...
    for (const doc of coachAccountsSnap.docs) {
      ...
      try {
        ...
        for (const session of sessions.data) {
          ...
          const sessionCreatedAt = Timestamp.fromMillis((session.created ?? Date.now() / 1000) * 1000);
          const contractEndAt = Timestamp.fromMillis(
            sessionCreatedAt.toMillis() + contractMonths * 30.44 * 24 * 60 * 60 * 1000
          );
          ...
          if (session.subscription) {
            ...
            if (!existingSubSnap.exists) {
              const tierSplit = parseInt(session.metadata?.tierSplit ?? '40', 10) as 40 | 35 | 30;
...
export const setProfitShareStartDate = onCall(
  ...
  async (request) => {
    ...
    if (!coachId || !startDate) {
      throw new HttpsError('invalid-argument', 'coachId and startDate (YYYY-MM-DD) are required');
    }

    // Validate date format
    const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
    if (!dateRegex.test(startDate)) {
      throw new HttpsError('invalid-argument', 'startDate must be in YYYY-MM-DD format');
    }
...
export const getConnectedAccountData = onCall(
  ...
  async (request) => {
    ...
    const stripeAccountId = coachAccountSnap.data()?.stripeAccountId as string | undefined;
    if (!stripeAccountId) {
      throw new HttpsError('not-found', 'Coach has no connected Stripe account');
    }
    ...
    return {
      ...
      subscriptions: subscriptions.data.map(s => ({
        ...
        items: s.items.data.map(i => ({
          ...
          interval: typeof i.price === 'string' ? null : i.price.recurring?.interval,
        })),
      })),
      invoices: invoices.data.map(i => ({
        ...
        created: i.created,
        periodStart: (i as any).period_start,
        periodEnd: (i as any).period_end,
      })),
      charges: charges.data.map(c => ({
        ...
        applicationFeeAmount: (c as any).application_fee_amount,
      })),
      balance: {
...
export const generateVoice = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      throw new HttpsError('internal', `voice:storage:upload_failed`, {
        layer: 'storage',
        bucket: bucket.name,
        path,
        code,
...
export const resolveShareToken = onRequest(
  ...
  async (req, res) => {
    ...
    const teaser = {
      ...
      coachId: tokenData.createdBy as string,
      tags: workout.tags || [],
      visibility,
      requireAuth,
...
export const generateEquipmentImage = onCall(
  { region: 'us-central1', secrets: [openaiApiKey], timeoutSeconds: 300, invoker: 'public' },
  async (request) => {
    ...
    async function generateOne(variantIndex: number): Promise<Buffer> {
      ...
      throw new HttpsError('internal', `Image generation failed (variant ${variantIndex + 1}): ${lastApiError}`);
    }
