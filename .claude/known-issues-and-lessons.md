...
### CI Player Journey Harness Must Be Derived from Actual Deployed Source (PR #370)
When writing or correcting a browser-automation harness for a multi-screen player journey, every screen's preconditions and order must be derived by reading the actual deployed candidate's source code — not from PR descriptions, review notes, or developer assumptions. PR #370 found that the hosted player journey was asserting a screen sequence (`event-title` immediately after the cold scan) that the product does not have: on a cold scan the device-question and signed-out screens necessarily precede event-title.

Additionally, a journey harness "pass" must prove the product's behavior, not the harness's own navigation capability. The prior `reachMemberEvent` helper navigated back to the event page after sign-in and then asserted the product had NOT returned — which passed on every build, including broken ones. The corrected helper performs exactly one navigation (the cold scan), then waits for the product's own URL transition. A build where the product does not return the user to the scanned event fails the test.

Two corollaries from this PR: **never count occurrences of a string against the raw file text** when the file contains comments that could quote code verbatim — strip comments first (a dedicated `codeOfFn` helper). And **a harness check written against a state that never existed in production still needs its own mutation test** — a pinned check that is never violated is not evidence the check is working.

Lesson: before writing any step in a player journey harness, read the actual component and route source at the candidate SHA being tested. PR descriptions describe intent; source code describes reality. Assert the product's outcome from outside — do not perform navigation inside the helper and then claim the product did it.

### CI Player Journey Locators Must Target the Single Visible DOM Copy (PRs #375, #376)
When a multi-participant player journey renders the same semantic element in more than one DOM location — for example, an event title present in the visible route component and also in a background-mounted or preloaded sibling — a locator that matches any copy will produce spurious passes when the visible copy is absent. PR #375 found that the hosted player journey was waiting for the event-title element without scoping to the visible root; a hidden background mount could satisfy the locator even when the correct screen had not appeared. PR #376 found that post-arrival activity and queue controls were asserted without scoping to the member-event root, allowing off-screen copies to satisfy assertions that require on-screen presence.

Lesson: in any multi-step player journey, scope every locator to the route or component root that is actually visible at that step. A test that selects an element by role or data attribute alone may match the wrong DOM subtree and produce a green result that proves nothing about the visible state. When a harness row produces spurious passes or unexpected timeouts, check whether the locator is scoped to the visible element tree before debugging the environment or the product.