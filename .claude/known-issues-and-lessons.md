...
### The Canvas PiP Path Was Always-On and Starved Foreground Music — Fixed in PR #289
**Before PR #289**, `usePipCanvasStream` was not gated on whether PiP was actually active: `enabled: pipEnabled` passed `true` whenever the staging flag was on, so the 30fps `requestAnimationFrame` loop, canvas draws, and `captureStream` ran continuously from player mount. Device-verified consequence (2026-08-15): the always-on canvas hook starved the foreground `musicGain → destination` audio path, silencing workout music even when PiP was not open. **PR #289 fixed this** by hoisting `isPiP` state above the canvas hook and passing it as `enabled: isPiP`, so the rAF loop and hidden video only run while PiP is genuinely active.

**The earlier "not gated on iOS" analysis (documented in a prior version of this entry) was correct but incomplete.** The `hasWorkingCaptureStream` UA-sniff computed in the hook had zero consumers — the flag was defined and returned but never read by `WorkoutPlayer.tsx`. That remains true after #289. The `isPiP` gate is what now controls activity; the UA-sniff is still dead code. Any future refactor of the canvas hook should either wire `hasWorkingCaptureStream` to something or remove it.

**Three lessons from this arc.** A flag that is computed but never read is not a gate — grep for consumers before describing any code path as disabled. An always-on hook that touches audio-affecting APIs must prove it cannot starve other audio paths (see the media-timer busy-guard lesson). And the cheapest diagnostic for "foreground audio went silent" is gating the most recent audio-affecting hook addition — before reaching for oscilloscopes.
...
### Pricing: `||` Swallows Explicit Zeros — Use `??`
Found in production (PR #293, 2026-08-18): a live coach's plan with phases `[0, 0, 13]` (all 13 weeks in phase 3, Self-Reliant) was quoting ~$421/mo instead of ~$176/mo. `_calculatePricing` used `||` to fall back from a phase's week count: `(phases[0]?.weeks) ? weeks : Math.round(totalWeeks * 0.25)`. Since `0` is falsy in JavaScript, an explicit zero silently reinflated to the 25/50/25 default split. The row-level breakdown read zeros correctly, so the displayed breakdown rows never summed to the displayed total — the only visible symptom was an implausible quote.

Fix: `?? 0` (nullish coalescing) instead of `|| default`. The fallback fires only when the value is `null` or `undefined`, not when it is a legitimate zero.

Lesson: any numeric field that is legitimately allowed to be zero must use `?? fallback` rather than `|| fallback`. The risk is highest in pricing, duration, and capacity calculations where zero is a valid meaningful value (not just "unset"). A sibling factory function that builds defaults from scratch is a separate code path and does not carry the same risk, but every consumption of real coach data must use `??`.
...
### Plan Scenarios: Shared-Plan Projections Via Cloud Function Trigger, Not Direct Rules Access
When a subcollection (`scenarios`) must remain private to authenticated users but unauthenticated share-link visitors need a read-only view of the presented scenario, do not relax the subcollection rules. Instead (PR #298), use a Firestore trigger (`sharedPlanViews`) to write token-keyed public projections whenever `presentedScenarioId` changes — unauthenticated readers consume the projection, never the subcollection. This is the trigger-written-mirror variant of the callable-projection pattern: same privacy guarantee, but the data is pushed by a server-side trigger rather than pulled on-demand by a callable. Use when the projected surface must be fast-fresh (no per-request callable latency) and the write cardinality is low.
...
### Second App in Monorepo: Zero-Custom-Claims Guard Is Mandatory
WestayFit (WSF) ships from the same `goarrive` Firebase project as GoArrive. A critical isolation invariant: **WSF must never call `setCustomUserClaims`** — 7 of 8 GoArrive claim-writer paths would silently clobber any WSF-set claim on the same UID if a user signs in to both apps.

This is enforced by a Vitest static guard in `apps/westayfit/tests/zero-custom-claims.test.ts`: at test time it runs `git grep setCustomUserClaims` scoped to WSF-owned paths (`apps/westayfit/`, `functions-westayfit/`) and fails if any match is found. The guard runs in CI so no WSF PR can accidentally introduce a claims call.

Lesson: in a shared-Firebase-project monorepo, custom claims are a shared namespace and a silent clobber surface. Any second app in the repo that handles Firebase Auth must either (a) use a completely disjoint claims key space and coordinate with the primary app, or (b) commit to never touching claims and enforce that with a static guard. Option (b) is cheaper and safer when the second app does not need per-user roles on the primary app's claims structure.
