...
**Billing Foundation**: Stripe Connect (Standard mode) is integrated with checkout sessions, webhook handling, subscription management, dispute handling, and ledger entries. Prorated earnings caps with yearly admin configuration and automatic carryover are implemented. Free ($0) plans are now supported end-to-end: the pricing engine allows $0, free plans activate immediately (no payment required), and a card-on-file CTS path is available for when the plan upgrades. A post-payment claim gate handles account creation and plan linking for members who pay before creating an account.
...
**Admin Operations**: Coach management, profit share settings, yearly earnings cap configuration, per-coach module visibility (feature gating, including hiding the tab bar entirely when all gateable modules are off), dead letter queue, event log, and system health monitoring are available. An **Admin Leads tab** surfaces unassigned intake submissions with automated alerts (email + Slack) so the platform admin can route new leads to coaches.
...
An iPad canvas-scale cap prevents the countdown badge from blowing up on large screens (artboard max-scale capped).
...
**AI Features**: Movement variations via Runway (build a variation from an existing movement, background-mode polling that persists outputs, motion remix from a still frame), AI workout music via Mubert (18 styles, no-repeat track pools that change songs through the workout with a deterministic per-workout order, in-player music panel with skip/like/dislike/volume/style-change, per-user `musicPrefs` + shared per-workout `workoutMusicFeedback` dislikes; volume now uses a Web Audio GainNode for true iOS volume control (HTMLMediaElement volume is ignored on iOS Safari); perceptual volume curve with a hard floor at 0; music and TTS/voice use separate gain nodes so the music slider does not affect spoken cues; share-link visitors get anonymous auth so music plays without a login; music fields are sanitized for the share-link path), AI-generated spoken workout intro announcements, grab-equipment AI images, and follow-along video blocks.
...
**Coach Discovery**: A new coach discovery experience is live — coaches can be surfaced to prospective members through a sanitized discovery flow with legal verification and opportunity/invitation UI. A dedicated hosting config isolates the discovery routes.

**Member Account Linking**: The coach UI now surfaces member account link status and provides linking feedback when an intake lead has not yet created an account, so coaches can identify and action unlinked members.
...
## What Is In Flight (Open PRs as of 2026-08-11)
| PR | Area | Status |
|---|---|---|
